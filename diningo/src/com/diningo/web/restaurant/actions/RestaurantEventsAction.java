 ...
 *                                                                           *
 *****************************************************************************
 * $Id: RestaurantEventsAction.java,v 1.1 2008/01/22 22:02:51 manish Exp $
 *****************************************************************************
 */
 ...
 *             @author   Manish Gupta
 *            @version   1.0
 *    Date of creation : 13-08-2007
 *    Last Modified by : 
 *  Last Modified Date :
 */
public class

RestaurantEventsAction extends LookupDispatchAction {
    ...
    public ActionForward save(ActionMapping mapping, 
        ...
        try {
            ...
            if (RestaurantEventsForm.getHdnRestaurantEventsTblPk() == null || "".equals(RestaurantEventsForm.getHdnRestaurantEventsTblPk())) {
                rc = Operations.AddRestaurantEvents(RestaurantEventsForm,ds);
                if (rc < 0) {
                   actionErrors.add("user",new ActionMessage("errors.restaurant.events.insert.error"));
                }
            }
            else {
                rc = Operations.UpdateRestaurantEvents(RestaurantEventsForm,ds);
                if (rc < 0) {
                   actionErrors.add("user",new ActionMessage("errors.restaurant.events.update.error"));
                }
            }
            if (rc < 0) {
    ...
    public ActionForward delete(ActionMapping mapping, 
        ...
        try {
            ...
            rc = Operations.DeleteRestaurantEvents(RestaurantEventsForm,ds);
            if (rc < 0) {
                actionErrors.add("user",new ActionMessage("errors.restaurant.events.delete.error"));
                saveErrors(request, actionErrors);
            }
        } catch (Exception e) {
          logger.error(e.toString());
